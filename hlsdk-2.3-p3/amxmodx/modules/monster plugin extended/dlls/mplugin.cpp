...
mBOOL MPlugin::ini_parseline(char *line) {
	...
	// grab filename
	token=strtok_r(NULL, " \t\r", &ptr_token);
	if(!token)
		RETURN_ERRNO(mFALSE, ME_FORMAT);
	...
	// Grab description.
	// Just get the the rest of the line, minus DOS line-ending "^M".
	token=strtok_r(NULL, "\r", &ptr_token);
	if(token) {
	...
	if(is_absolute_path(filename))
		STRNCPY(pathbuf, filename, sizeof(pathbuf));
	else
		snprintf(pathbuf, sizeof(pathbuf), "%s/%s", GameDLL.gamedir, filename);
	// Remove relative path components, if possible.
	if(!realpath(pathbuf, pathname)) {
		META_ERROR("Unable to get realpath for '%s': %s", token,
				str_os_error());
		STRNCPY(pathname, pathbuf, sizeof(pathname));
	}
...
mBOOL MPlugin::query(void) {
	...
	if(info && !FStrEq(info->ifvers, META_INTERFACE_VERSION)) {
		...
		META_LOG("dll: Note: Plugin '%s' interface version didn't match; expected %s, found %s", 
				desc, META_INTERFACE_VERSION, info->ifvers);
		...
		if(pmajor > mmajor || (pmajor==mmajor && pminor > mminor)) {
			META_ERROR("dll: Plugin '%s' requires a newer version of Metamod", 
					desc);
		...
		// (update plugin).
			...
			META_ERROR("dll: Plugin '%s' is out of date and incompatible with this version of Metamod; please find a newer version of the plugin", desc);
			meta_errno=ME_IFVERSION;
		}
		...
		// backwards compatible, so we warn, but we still accept it.
			...
			META_LOG("dll: Note: plugin '%s' is not using the latest interface version; there may be an updated version of the plugin", desc);
		else
	...
	if(meta_errno == ME_IFVERSION) {
		META_ERROR("dll: Rejected plugin '%s' due to interface version incompatibility", desc);
