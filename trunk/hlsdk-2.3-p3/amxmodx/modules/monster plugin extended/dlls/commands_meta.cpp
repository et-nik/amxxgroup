...
void cmd_doplug(PLUG_CMD pcmd) {
	...
	// Yes, the command string gets mapped to an enum above, and then
	// mapped back to a string here, which may not be the cleanest design.
	// The idea was to avoid lots of strcmps..
	if(pcmd==PC_PAUSE)
		cmd="pause";
		...
		META_ERROR("Unknown plug_cmd: %d", pcmd);
	...
	// H=2 to skip first arg, as it should match 'cmd'
	for(i=2; i < argc; i++) {
		arg=CMD_ARGV(i);
