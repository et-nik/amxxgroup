...
{
   if (( strcmp(STRING(pTarget->pev->model), "models/agrunt.mdl") == 0 ) || ( strcmp(STRING(pTarget->pev->model), "models/garg.mdl") == 0 ))
    {
		return R_NM;
	}
...
void CMHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (ptr->iHitgroup == 11)
	{
		...
		if (GetBodygroup( 1 ) == HEAD_GRUNT && (bitsDamageType & (DMG_NEVERGIB | DMG_SLASH | DMG_BLAST | DMG_CLUB)))
		{
			// absorb damage
			flDamage -= 20;
			if (flDamage <= 0)
