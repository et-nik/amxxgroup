...
const char *CMIchthyosaur::pIdleSounds[] = 
{
	"ichy/ichy_idle1.wav",
	"ichy/ichy_idle2.wav",
	"ichy/ichy_idle3.wav",
...
const char *CMIchthyosaur::pAlertSounds[] = 
{
	"ichy/ichy_alert2.wav",
	"ichy/ichy_alert3.wav",
};

const char *CMIchthyosaur::pAttackSounds[] = 
{
	"ichy/ichy_attack1.wav",
	"ichy/ichy_attack2.wav",
};

const char *CMIchthyosaur::pBiteSounds[] = 
{
	"ichy/ichy_bite1.wav",
	"ichy/ichy_bite2.wav",
};

const char *CMIchthyosaur::pPainSounds[] = 
{
	"ichy/ichy_pain2.wav",
	"ichy/ichy_pain3.wav",
	"ichy/ichy_pain5.wav",
...
const char *CMIchthyosaur::pDieSounds[] = 
{
	"ichy/ichy_die2.wav",
	"ichy/ichy_die4.wav",
};
...
void CMIchthyosaur :: IdleSound( void )	
{ 
	EMIT_ICKY_SOUND( CHAN_VOICE, pIdleSounds ); 
}

void CMIchthyosaur :: AlertSound( void ) 
{ 
	EMIT_ICKY_SOUND( CHAN_VOICE, pAlertSounds ); 
}

void CMIchthyosaur :: AttackSound( void ) 
{ 
	EMIT_ICKY_SOUND( CHAN_VOICE, pAttackSounds );
}

void CMIchthyosaur :: BiteSound( void ) 
{ 
	EMIT_ICKY_SOUND( CHAN_WEAPON, pBiteSounds );
}

void CMIchthyosaur :: DeathSound( void ) 
{ 
	EMIT_ICKY_SOUND( CHAN_VOICE, pDieSounds ); 
}

void CMIchthyosaur :: PainSound( void )	
{ 
...
DEFINE_CUSTOM_SCHEDULES(CMIchthyosaur)
{
    slSwimAround,
	slSwimAgitated,
	slCircleEnemy,
...
IMPLEMENT_CUSTOM_SCHEDULES(CMIchthyosaur, CMFlyingMonster);

//=========================================================
// Classify - indicates this monster's place in the 
// relationship table.
//=========================================================
int	CMIchthyosaur :: Classify ( void )
{
...
// CheckMeleeAttack1
//=========================================================
BOOL CMIchthyosaur :: CheckMeleeAttack1 ( float flDot, float flDist )
{
...
void CMIchthyosaur::BiteTouch( edict_t *pOther )
{
...
void CMIchthyosaur::CombatUse( edict_t *pActivator, edict_t *pCaller, USE_TYPE useType, float value )
{
...
//
//=========================================================
BOOL CMIchthyosaur :: CheckRangeAttack1 ( float flDot, float flDist )
{
...
//=========================================================
void CMIchthyosaur :: SetYawSpeed ( void )
{
	pev->yaw_speed = 100;
}
...
//=========================================================
// Killed - overrides CMFlyingMonster.
//
void CMIchthyosaur :: Killed( entvars_t *pevAttacker, int iGib )
{
	CMBaseMonster::Killed( pevAttacker, iGib );
	
}

void CMIchthyosaur::BecomeDead( void )
{
...
//=========================================================
void CMIchthyosaur :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case ICHTHYOSAUR_AE_SHAKE_LEFT:
		{
			if (m_hEnemy != NULL)
			{
				edict_t *pHurt = m_hEnemy;

				if (m_flEnemyTouched < gpGlobals->time - 0.2 && (UTIL_BodyTarget( m_hEnemy,pev->origin ) - pev->origin).Length() > (32+16+32))
					break;
				...
				if (DotProduct( vecShootDir, gpGlobals->v_forward ) > 0.707)
				{
					...
					pHurt->v.velocity = pHurt->v.velocity - gpGlobals->v_right * 300;
					if (UTIL_IsPlayer(pHurt))
					{
						pHurt->v.angles.x += RANDOM_FLOAT( -35, 35 );
						pHurt->v.angles.y += RANDOM_FLOAT( -90, 90 );
						pHurt->v.angles.z = 0;
						pHurt->v.fixangle = TRUE;
					}
					if (UTIL_IsPlayer(pHurt))
					{
						UTIL_TakeDamage( pHurt,pev, pev, gSkillData.ichthyosaurDmgShake, DMG_SLASH );
					}
					else if (pHurt->v.euser4 != NULL)
					{
						CMBaseMonster *pMonster = GetClassPtr((CMBaseMonster *)VARS(pHurt));
						pMonster->TakeDamage( pev, pev, gSkillData.ichthyosaurDmgShake, DMG_SLASH );
					}
				}
			}
			BiteSound();
...
// GetSchedule
//=========================================================
Schedule_t* CMIchthyosaur::GetSchedule()
{
	...
	return CMFlyingMonster :: GetSchedule();
}


//=========================================================
//=========================================================
Schedule_t* CMIchthyosaur :: GetScheduleOfType ( int Type ) 
{
...
void CMIchthyosaur::StartTask(Task_t *pTask)
{
	...
	case TASK_SMALL_FLINCH:
		...
		CMFlyingMonster::StartTask(pTask);
		break;
...
void CMIchthyosaur :: RunTask ( Task_t *pTask )
{
	...
	case TASK_ICHTHYOSAUR_CIRCLE_ENEMY:
		...
		else if (UTIL_IsPlayer( m_hEnemy ))
		{
			Vector vecFrom = UTIL_EyePosition(m_hEnemy );
			...
			UTIL_TraceHull( vecFrom, vecPos, ignore_monsters, large_hull, m_hEnemy, &tr );
			...
			if (HasConditions( bits_COND_ENEMY_FACING_ME ) && UTIL_IsPlayer( m_hEnemy))
			{
				m_flNextAlert -= 0.1;
...
float CMIchthyosaur::VectorToPitch( const Vector &vec )
{
	float pitch;
	if (vec.z == 0 && vec.x == 0)
		pitch = 0;
...
//=========================================================
void CMIchthyosaur::Move(float flInterval)
{
	CMFlyingMonster::Move( flInterval );
}

float CMIchthyosaur::FlPitchDiff( void )
{
	float	flPitchDiff;
...
float CMIchthyosaur :: ChangePitch( int speed )
{
...
float CMIchthyosaur::ChangeYaw( int speed )
{
	if ( pev->movetype == MOVETYPE_FLY )
	{
	...
	return CMFlyingMonster::ChangeYaw( speed );
}


Activity CMIchthyosaur:: GetStoppedActivity( void )
{ 
...
void CMIchthyosaur::MoveExecute( edict_t *pTargetEnt, const Vector &vecDir, float flInterval )
{
	m_SaveVelocity = vecDir * m_flightSpeed;
}


void CMIchthyosaur::MonsterThink ( void )
{
	CMFlyingMonster::MonsterThink( );
...
void CMIchthyosaur :: Stop( void ) 
{
	if (!m_bOnAttack)
		m_flightSpeed = 80.0;
}

void CMIchthyosaur::Swim( )
{
	int retValue = 0;
...
Vector CMIchthyosaur::DoProbe(const Vector &Probe)
{
	...
	if (bBumpedSomething && (m_hEnemy == NULL || tr.pHit != m_hEnemy))
	{
