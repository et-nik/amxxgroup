...
const char *CMLeech::pAttackSounds[] =
{
...
const char *CMLeech::pAlertSounds[] =
{
...
void CMLeech::RecalculateWaterlevel( void )
{
...
void CMLeech::SwitchLeechState( void )
{
	...
	{
		Look( m_flDistLook );
		edict_t *pEnemy = BestVisibleEnemy();
		if ( pEnemy && pEnemy->v.waterlevel != 0 )
		{
			m_hEnemy = pEnemy;
...
int CMLeech::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
...
void CMLeech::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
	case LEECH_AE_ATTACK:
		...
		if ( pEnemy != NULL )
		{
			...
			face.z = 0;
			dir = (pEnemy->v.origin - pev->origin);
			dir.z = 0;
			dir = dir.Normalize();
			face = face.Normalize();
			...
			if ( DotProduct(dir, face) > 0.9 )	
			{
				...
				{
					UTIL_TakeDamage( pEnemy,pev, pev, gSkillData.leechDmgBite, DMG_GENERIC );
				}
			else if (pEnemy->v.euser4 != NULL)
				{
					CMBaseMonster *pMonster = GetClassPtr((CMBaseMonster *)VARS(pEnemy));
					pMonster->TakeDamage( pev,pev, gSkillData.leechDmgBite, DMG_GENERIC );
				}
			}
		}
		m_stateTime -= 2;
...
//
float CMLeech::ObstacleDistance( edict_t *pTarget )
{
	TraceResult		tr;
	Vector			vecTest;
...
//		UTIL_SetOrigin( pev, pev->oldorigin );
	...
	if ( tr.flFraction != 1.0 )
	{
		if ( (pTarget == NULL || tr.pHit != pTarget) )
		{
...
void CMLeech::UpdateMotion( void )
{
...
void CMLeech::SwimThink( void )
{
	...
	case MONSTERSTATE_COMBAT:
		...
		else
		{
			...
			m_height = pTarget->v.origin.z + pTarget->v.view_ofs.z - 5;
			// Clip to viable water area
			if ( m_height < m_bottom )
				m_height = m_bottom;
			else if ( m_height > m_top )
				m_height = m_top;
			Vector location = pTarget->v.origin - pev->origin;
			location.z += (pTarget->v.view_ofs.z);
...
void CMLeech::Killed(entvars_t *pevAttacker, int iGib)
{
	Vector			vecSplatDir;
	TraceResult		tr;
