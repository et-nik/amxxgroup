...
void metamod_startup(void) {
	...
	// Set a slight debug level for developer mode.
	if((int) CVAR_GET_FLOAT("developer") != 0)
	...
	if((cp=LOCALINFO("mm_debug")) && *cp != '\0') {
		int d = atoi(cp);
		META_LOG("mm_debug: Setting meta_debug to %d", d);
		CVAR_SET_FLOAT("meta_debug", d);
	}
	...
	// First, look for "localinfo mm_pluginsfile <file listing mm plugins>".
	if((mmfile=LOCALINFO("mm_pluginsfile")) && *mmfile != '\0') {
		if(valid_gamedir_file(mmfile))
			META_LOG("Plugins file specified via localinfo: %s", mmfile);
		else {
			META_ERROR("Plugins file from localinfo is empty/missing: %s", 
					mmfile);
			mmfile=NULL;
		}
	}
	...
	//  - $gamedir/metamod.ini
	//
	// If neither exists, fall back to former, so that that path will be 
	// used in the error message from ini_startup().
	if(!mmfile) {
		if(valid_gamedir_file(PLUGINS_INI))
			mmfile=PLUGINS_INI;
		else if(valid_gamedir_file(OLD_PLUGINS_INI))
	...
	// messages.
	// First, look for "localinfo mm_execcfg <file listing mm plugins>".
	if((mmfile=LOCALINFO("mm_execcfg")) && *mmfile != '\0') {
		if(valid_gamedir_file(mmfile))
			META_LOG("Metamod exec.cfg specified via localinfo: %s", mmfile);
		else {
			META_ERROR("Metamod exec.cfg from localinfo is empty/missing: %s", 
					mmfile);
			mmfile=NULL;
		}
	}
	...
	//
	// If neither exists, fall back to former, just because..
	//
	// Yes, this all uses more LOAD_FILE calls than is strictly necessary.
	if(!mmfile) {
		if(valid_gamedir_file(EXEC_CFG))
			mmfile=EXEC_CFG;
	...
	cp = (char *) LOAD_FILE_FOR_ME(mmfile, &len);
	if(cp && len) {
		char cmd[NAME_MAX];
