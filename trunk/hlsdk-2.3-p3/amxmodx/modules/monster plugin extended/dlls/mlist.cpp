...
MPlugin *MPluginList::find(const char *findpath) {
	...
	if(!findpath)
		RETURN_ERRNO(NULL, ME_ARGUMENT);
	META_DEBUG(8, ("Looking for plugin dlfnamepath: %s", findpath));
	for(i=0; i < endlist; i++) {
		META_DEBUG(9, ("Trying: plugin %s loadedpath: %s", plist[i].file, plist[i].pathname));
		if(plist[i].status < PL_VALID)
			continue;
		if(strmatch(plist[i].pathname, findpath)) {
			META_DEBUG(8, ("Found! plugin %s", plist[i].file));
			return(&plist[i]);
		}
	}
	META_DEBUG(8, ("Failed! no pluginpath: %s", findpath));
	RETURN_ERRNO(NULL, ME_NOTFOUND);
}
...
// Find a plugin with non-ambiguous prefix string matching desc, file, or
// name.
// meta_errno values:
...
mBOOL MPluginList::ini_startup() {
	...
	cfg=(char *)LOAD_FILE_FOR_ME(inifile, &len);
	if(!cfg || !len) {
		META_ERROR("ini: Metamod plugins file empty or missing: %s/%s", 
				GameDLL.gamedir, inifile);
		RETURN_ERRNO(mFALSE, ME_NOFILE);
	}

	META_LOG("ini: Begin reading plugins list: %s", inifile);
	for(line=strtok_r(cfg, "\n", &ptr_line), n=0;
			line && n < size; 
			line=strtok_r(NULL, "\n", &ptr_line)) 
	{
		...
		if(!plist[n].ini_parseline(line)) {
			if(meta_errno==ME_FORMAT)
				META_ERROR("ini: Skipping malformed line %d of %s", n, inifile);
			continue;
		}
